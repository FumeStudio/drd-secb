...
public class GalleryFragment extends SECBBaseFragment
    ...
    private List<GalleryItem> photoGalleryItemList;
    private List<GalleryItem> videoGalleryItemList;
...
//		UiEngine.applyCustomFont(((TextView) view.findViewById(R.id.textViewAbout)), UiEngine.Fonts.HELVETICA_NEUE_LT_STD_CN);
    ...
    private void loadMoreData() {
        ...
        if(galleryType == GalleryItem.GALLERY_TYPE_VIDEO_GALLERY)
            startGalleryListOperation(GalleryItem.GALLERY_TYPE_VIDEO_GALLERY, false, (PagingManager.getLastPageNumber((ArrayList<GalleryItem>)videoGalleryItemList) + 1));
        else
            startGalleryListOperation(GalleryItem.GALLERY_TYPE_IMAGE_GALLERY, false, (PagingManager.getLastPageNumber((ArrayList<GalleryItem>)photoGalleryItemList) + 1));
    }
...
//		        else
    ...
    private void startGalleryListOperation(int galleryTypeVideoGallery, boolean showDialog, int pageIndex) {
        GalleryOperation operation = null;
        if (galleryTypeVideoGallery == GalleryItem.GALLERY_TYPE_IMAGE_GALLERY)
            operation = new GalleryOperation(GalleryItem.GALLERY_TYPE_IMAGE_GALLERY, RequestIds.PHOTO_GALLERY_REQUEST_ID, showDialog, getActivity(), ServerKeys.PAGE_SIZE_DEFAULT, pageIndex);

        else if (galleryTypeVideoGallery == GalleryItem.GALLERY_TYPE_VIDEO_GALLERY)
            operation = new GalleryOperation(GalleryItem.GALLERY_TYPE_VIDEO_GALLERY, RequestIds.VIDEO_GALLERY_REQUEST_ID, showDialog, getActivity(), ServerKeys.PAGE_SIZE_DEFAULT, pageIndex);
    ...
    public void handleRequestFinished(Object requestId, Throwable error, Object resultObject) {
        ...
        if (error == null) {
            ...
            if ((int) requestId == RequestIds.PHOTO_GALLERY_REQUEST_ID && resultObject != null) {
                ArrayList<GalleryItem> _photoGalleryItemList = (ArrayList<GalleryItem>) resultObject;
                int pageIndex = PagingManager.getLastPageNumber(_photoGalleryItemList);
                if (photoGalleryItemList == null || pageIndex == 0)
                    photoGalleryItemList = new ArrayList<>();
                photoGalleryItemList.addAll(_photoGalleryItemList);
            ...
            //videoGallery
                ...
                ArrayList<GalleryItem> _videoGalleryItemList = (ArrayList<GalleryItem>) resultObject;
                int pageIndex = PagingManager.getLastPageNumber(_videoGalleryItemList);
                if (videoGalleryItemList == null || pageIndex == 0)
                    videoGalleryItemList = new ArrayList<>();
                videoGalleryItemList.addAll(_videoGalleryItemList);
